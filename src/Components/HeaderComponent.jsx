    ...
    render() {
       ...
       const isUserAdmin=AuthenticationService.isUserAdmin();
        return (
            ...
            <header>
        <nav className="navbar navbar-expand-md navbar-dark bg-dark" style={{backgroundColor:"#e3f2fd"}}>
       <div className="navbar-brand">SrcQwikbot</div>
       <ul className="navbar-nav">
         {isUserloggedIN &&<button  type="button" className="btn btn-light mr-2"><Link to="/Welcome/-----">Home</Link></button>}
         {isUserloggedIN &&<button  type="button" className="btn btn-light mr-2"><Link to="/Chatbot">Chatbot</Link></button>}
